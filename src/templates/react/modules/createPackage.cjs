...
async function createPackageJson(projectPath, options) {
  ...
  // 根据选择的UI框架添加依赖
  if (isAntd) {
    dependencies['antd'] = '^5.8.5';
    dependencies['@ant-design/icons'] = '^5.2.5';
  } else if (isChakra) {
    dependencies['@chakra-ui/react'] = '^2.8.2';
    dependencies['@chakra-ui/icons'] = '^2.1.1';
  ...
  if (options.unitTest === 'Jest') {
    ...
    devDependencies['@testing-library/jest-dom'] = '^6.1.3';
    devDependencies['@babel/preset-env'] = '^7.22.10';
    devDependencies['@babel/preset-react'] = '^7.22.5';
    if (isTS) {
...
这是一个使用前端脚手架生成的 React 项目，基于以下技术栈：
...
│   ├── store/           # Redux状态管理
│   ├── utils/           # 工具函数
│   ├── App.${options.language === 'TypeScript' ? 'tsx' : 'jsx'}         # 根组件
│   └── index.${options.language === 'TypeScript' ? 'tsx' : 'jsx'}       # 入口文件
...
- **状态管理**: 使用 Redux Toolkit 进行集中状态管理
...
<html lang="zh-CN">
  ...
  <body>
    <div id="root"></div>
    <script type="module" src="/src/index.${options.language === 'TypeScript' ? 'tsx' : 'jsx'}"></script>
  </body>
</html>
`;
